...
![Status](https://img.shields.io/badge/status-4%2F4%20bases%20em%20opera%C3%A7%C3%A3o-success)
...
Automatizar, para cada uma das 4 bases, o ciclo completo:
...
|---|---|
| Bases automatizadas | 4 (`meta_financiamento_seguro`, `numero_contratos`, `dias_sem_producao`, `carteira_parceiros`) |
...
main.py  (CLI / orquestrador)
   ...
   │
   └── sharepoint_sync.py
           └── implementado no código, mas hoje NENHUMA das 4 bases o aciona
...
| [sharepoint_sync.py](sharepoint_sync.py) | Download/upload via SharePoint (Office365-REST-Python-Client) — presente no código, não usado pelas 4 bases atuais |
...
```text
rpa_c6_veiculos/
├── config.py              # As 4 bases: caminho no Looker, filtros, colunas, pastas, frequência
...
├── sharepoint_sync.py     # Download/upload no SharePoint — não usado pelas 4 bases atuais
├── main.py                # CLI / orquestrador
├── requirements.txt       # Dependências do projeto
├── .env.example           # Modelo de variáveis de ambiente
├── downloads/              # Arquivos baixados do Looker (gerada em runtime)
├── staging/                # Bases originais durante o processamento via SharePoint (gerada em runtime, não usada pelas 4 bases atuais)
...
- **Falha conhecida — Dias sem Produção (SLA)**: apresenta timeout intermitente esperando a tabela carregar após aplicar o filtro. Quando ocorre, o log registra explicitamente que **é uma falha técnica de navegação/carregamento, não ausência de dados** para o período — a causa raiz ainda está sob investigação.
...
| `--all` | Rodar as 4 bases em sequência, com um único login |
